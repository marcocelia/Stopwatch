...
/**
  ...
  * @retval None
  */
void Lcd_Touch_Auto_Calibration(void){
	adjust_Para.xOffset=-353;
	adjust_Para.xScale=-0.089787;
	adjust_Para.yOffset=14;
	adjust_Para.yScale=0.063111;
}
...
#define CURSOR_LEN    (10)
  ...
  while (!adjust_OK_Falg) {
    ...
    if (tst_point.x > (point_Base[4].x + CALIBRATION_RANGE)
        ...
        || tst_point.y < (point_Base[4].y - CALIBRATION_RANGE)) {
      adjust_OK_Falg = 0;
      LCD_DisplayStringLine(LINE(8),"   Calibration Fail!    ", 0);
      delay(200);
      LCD_Clear(White);
      delay(300);
    } else {
  ...
  LCD_DisplayStringLine(LINE(8),"   Calibration OK!    ", 0);
  delay(200);
}
...
void Calibration_Test_Dispose(void) 
{
  ...
  LCD_DisplayStringLine(LINE(9),"        please touch the screen          ", 0);
  while (1) {
    do {
      pstate = IOE_TS_GetState();
      delay(10);
    ...
    LCD_DisplayStringLine(LINE(4),text,0);
    sprintf((char*)text,"   y_AD = %d ",point_new.y);
    LCD_DisplayStringLine(LINE(5),text,0);
    ...
    LCD_DisplayStringLine(LINE(7),text,0);
    sprintf((char*)text,"   y_POS = %d ", point_new.y);
    LCD_DisplayStringLine(LINE(8),text,0);
    LCD_SetTextColor(Red); 
...
unsigned int GetTouch_SC_async (unsigned int *xs, unsigned int *ys){
	TS_STATE *pstate;
	unsigned int xt;
	unsigned int yt;
	...
	if (pstate->TouchDetected){
	   ...
	   if ((xt > TOUCH_AD_VALUE_MAX)
	        || (xt < TOUCH_AD_VALUE_MIN)
	        || (yt > TOUCH_AD_VALUE_MAX)
	        || (yt < TOUCH_AD_VALUE_MIN))
	      return 0;
	   *xs = ((int)(xt * adjust_Para.xScale - adjust_Para.xOffset));
	   *ys = ((int)(yt * adjust_Para.yScale - adjust_Para.yOffset));

	   if (xt >= LCD_PIXEL_WIDTH) {
	         xt = LCD_PIXEL_WIDTH -1;
	       }
	   if (yt >= LCD_PIXEL_HEIGHT) {
	         yt = LCD_PIXEL_HEIGHT -1;
	       }
