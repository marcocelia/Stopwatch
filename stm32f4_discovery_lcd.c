...
/*MODICA CELIA NUOVA CLEAR LINEA PARZIALE*/
void LCD_ClearLineRegion(uint16_t Line,uint16_t startx, uint16_t endx){
	uint16_t start_refcolumn = startx*LCD_Currentfonts->Width;
	uint16_t tmp=start_refcolumn+endx*LCD_Currentfonts->Width;
    uint16_t end_refcolumn = (tmp>LCD_PIXEL_WIDTH)?LCD_PIXEL_WIDTH:tmp;
	  do {
	       /* Display one character on LCD */
	    LCD_DisplayChar(Line, start_refcolumn, ' ');
	    /* Decrement the column position by 16 */
	    start_refcolumn += LCD_Currentfonts->Width;
	  } while (start_refcolumn <= end_refcolumn);
}
...
/**
  ...
  * @retval None
  */
void LCD_DisplayStringLine(uint16_t Line, uint8_t *ptr, uint8_t start_col)
{
  uint16_t refcolumn = start_col*LCD_Currentfonts->Width;
